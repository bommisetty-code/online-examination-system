...
class E2EIntegrationTests:
    ...
    def test_admin_workflow(self):
        ...
        for i in range(2):
            r = self.client.post(f'/api/admin/exams/{exam_id}/questions',
                                ...
                                json={'question_text': f'Question {i+1}?',
                                      'order_number': i+1, 'marks': 1})
            if r.status_code != 201:
                print(f"    ✗ FAIL: Add question {r.status_code}")
                return False
        print(f"    ✓ Added 2 questions")
        
        print("  3. Add options to questions...")
        # Get questions
        r = self.client.get(f'/api/admin/exams/{exam_id}/questions',
                           headers=self._h(self.admin_token))
        questions = r.get_json()['questions']
        
        for q in questions:
            # Add correct option
            r = self.client.post(f'/api/admin/exams/{exam_id}/questions/{q["id"]}/options',
                                headers=self._h(self.admin_token),
                                json={'option_text': 'Correct', 'option_order': 1,
                                      'is_correct': True})
            if r.status_code != 201:
                print(f"    ✗ FAIL: Add option {r.status_code}")
                return False
            
            # Add incorrect option
            r = self.client.post(f'/api/admin/exams/{exam_id}/questions/{q["id"]}/options',
                                headers=self._h(self.admin_token),
                                json={'option_text': 'Wrong', 'option_order': 2,
                                      'is_correct': False})
        print(f"    ✓ Added options to all questions")
        ...
        if r.status_code != 200:
            ...
            return False
        print(f"    ✓ Student 1 completed exam")
        ...
        if len(results) == 0:
            ...
            return False
        print(f"    ✓ Viewed results (found {len(results)} submission(s))")
    ...
    def test_student_workflow(self):
        ...
        r = self.client.post(f'/api/admin/exams/{exam_id}/questions',
                            headers=self._h(self.admin_token),
                            json={'question_text': 'Sample Q?', 'order_number': 1})
        q_id = r.get_json()['question']['id']
        
        # Add options
        self.client.post(f'/api/admin/exams/{exam_id}/questions/{q_id}/options',
                        headers=self._h(self.admin_token),
                        json={'option_text': 'A', 'option_order': 1, 'is_correct': True})
        self.client.post(f'/api/admin/exams/{exam_id}/questions/{q_id}/options',
                        headers=self._h(self.admin_token),
                        json={'option_text': 'B', 'option_order': 2, 'is_correct': False})
        print("  ✓ Exam created with questions")
        ...
        if target_exam['already_taken']:
            ...
            return False
        print(f"    ✓ Exam visible in student list")
        ...
        print(f"    ✓ Exam started (submission ID: {sub_id})")
        ...
        for q in questions:
            ...
            if len(opts) > 0:
                ...
                if r.status_code != 200:
                    ...
                    return False
        print(f"    ✓ Answered all questions")
        ...
        print(f"    ✓ Exam submitted (Score: {result['percentage']}%)")
        ...
        if r.status_code != 409:  # Conflict
            ...
            return False
        print(f"    ✓ Cannot retake exam (correctly rejected)")
        
        print("  ✓ STUDENT WORKFLOW COMPLETED")
        return True
    ...
    def test_student_refresh_during_exam(self):
        ...
        r = self.client.post(f'/api/admin/exams/{exam_id}/questions',
                            headers=self._h(self.admin_token),
                            json={'question_text': 'Q?', 'order_number': 1})
        q_id = r.get_json()['question']['id']
        
        r = self.client.post(f'/api/admin/exams/{exam_id}/questions/{q_id}/options',
                            headers=self._h(self.admin_token),
                            json={'option_text': 'O', 'option_order': 1, 'is_correct': True})
        ...
        print(f"    ✓ Refresh maintained session ({len(questions_2)} questions)")
        print("  ✓ REFRESH TEST COMPLETED")
        return True
